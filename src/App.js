...
// New component for confirm/alert dialogs
...
export default function App() {
    ...
    const [modalState, setModalState] = useState({ type: null, data: null }); // Updated modalState structure
    ...
    const startNewWorkout = (template = null) => {
        ...
        if (template) {
            ...
            // Ensure sets from template are displayed correctly based on current unit system
            initialExercises = JSON.parse(JSON.stringify(template.exercises)).map(ex => ({ 
                ...ex, 
                sets: ex.sets.map(set => ({ 
                    ...set, 
                    // Convert weight from stored (kg) to display units on load
                    weight: displayWeight(set.weight || 0), 
                    completed: false,
                    id: set.id || Date.now() + Math.random() // Ensure unique ID for active workout sets too
                })) 
            }));
        } else {
    ...
    const addSet = (exIndex) => {
        ...
        const defaultWeight = settings.unitSystem === 'imperial' ? displayWeight(storeWeight(22)) : 10;
    ...
    // Corrected `handleWorkoutSetChange` for active workout weight input
    const handleWorkoutSetChange = (exIndex, setIndex, field, value) => {
        const newWorkout = [...workout];
        if (field === 'weight') {
             // Convert the input value (which is in displayed units) back to stored units (kg)
             newWorkout[exIndex].sets[setIndex][field] = parseFloat(value) || 0; // Temporarily store as raw number, blur will finalize conversion
        } else {
    ...
    // Corrected `handleWorkoutSetBlur` for active workout weight input
    const handleWorkoutSetBlur = (exIndex, setIndex, field, value) => {
        const newWorkout = [...workout];
        let parsedVal = parseFloat(value) || 0;
        if (field === 'weight') {
            if (parsedVal > (settings.unitSystem === 'imperial' ? 660 : 300)) {
                parsedVal = settings.unitSystem === 'imperial' ? 660 : 300;
            }
            // Store the final converted value in kilograms
            newWorkout[exIndex].sets[setIndex][field] = storeWeight(parsedVal);
        } else {
    ...
    const handleAddExerciseToWorkout = (e) => {
        ...
        if (newExerciseName.trim()) {
            ...
            if (newExerciseMuscleGroup === 'Cardio') {
                newExercise.sets = [{ id: Date.now() + Math.random(), time: 10, completed: false }];
            } else {
                // Default weight based on unit system for active workout new exercise
                const defaultWeight = settings.unitSystem === 'imperial' ? 22 : 10; // Display value
                newExercise.sets = [{ id: Date.now() + Math.random(), reps: 8, weight: defaultWeight, completed: false }]; // Stored as kg
            }
    ...
    const handleEditTemplate = (template) => {
        ...
        // Ensure all exercises and their sets have unique IDs for React keys
        const exercisesWithIds = templateForEditing.exercises.map(ex => ({
            ...ex,
            sets: ex.sets.map(set => ({
                ...set,
                id: set.id || Date.now() + Math.random() // Assign a unique ID if missing
            }))
        }));
        setTemplateBuilderData({ ...templateForEditing, exercises: exercisesWithIds });
    ...
    const addExerciseToTemplateBuilder = (e) => {
        ...
        if (newExerciseName.trim() && templateBuilderData) {
            ...
            if (newExerciseMuscleGroup === 'Cardio') {
                newEx.sets = [{ id: Date.now() + Math.random(), time: 10 }]; // Add unique ID
            } else {
                // Default weight based on unit system for template new exercise
                const defaultWeight = settings.unitSystem === 'imperial' ? storeWeight(22) : storeWeight(10);
                newEx.sets = [{ id: Date.now() + Math.random(), reps: 8, weight: defaultWeight }]; 
            }
    ...
    const updateTemplateBuilderField = (exIndex, field, value) => {
        ...
        const defaultWeight = settings.unitSystem === 'imperial' ? storeWeight(22) : storeWeight(10); // Default weight in KG

        if (field === 'muscleGroup') {
            exercise[field] = value;
            if (value === 'Cardio') {
                exercise.sets = [{ id: Date.now() + Math.random(), time: 10 }]; // New set with ID for cardio
            } else if (!exercise.sets[0] || typeof exercise.sets[0].reps === 'undefined') {
                exercise.sets = [{ id: Date.now() + Math.random(), reps: 8, weight: defaultWeight }]; // New set with ID for non-cardio
            }
    ...
    // Corrected `handleTemplateBuilderSetChange` for weight input
    const handleTemplateBuilderSetChange = (exIndex, setIndex, field, value) => {
        const newExercises = [...templateBuilderData.exercises];
        if (field === 'weight') {
            // When changing weight, store it in the internal unit system immediately
            newExercises[exIndex].sets[setIndex][field] = storeWeight(parseFloat(value) || 0);
        } else {
            // For reps or time, parse as integer
            const parsedVal = parseInt(value, 10);
            newExercises[exIndex].sets[setIndex][field] = isNaN(parsedVal) ? '' : parsedVal;
        }
        setTemplateBuilderData(d => ({ ...d, exercises: newExercises }));
    };
    
    // Corrected `handleTemplateBuilderSetBlur` for weight input
    const handleTemplateBuilderSetBlur = (exIndex, setIndex, field, value) => {
        const newExercises = [...templateBuilderData.exercises];
        let parsedValue = parseFloat(value) || 0;

        if (field === 'weight') {
            // Apply max limit in the displayed unit, then store in internal unit
            if (parsedValue > (settings.unitSystem === 'imperial' ? 660 : 300)) {
                parsedValue = settings.unitSystem === 'imperial' ? 660 : 300;
            }
            newExercises[exIndex].sets[setIndex][field] = storeWeight(parsedValue);
        } else {
    ...
    const addSetToTemplateBuilder = (exIndex) => {
        ...
        // Determine default set based on muscle group, similar to adding a new exercise
        const newSet = exercise.muscleGroup === 'Cardio'
            ? { id: Date.now() + Math.random(), time: 10 } // Add unique ID for cardio set
            : { id: Date.now() + Math.random(), reps: 8, weight: defaultWeight }; // Add unique ID for non-cardio set
    ...
    const handleDeleteWeight = (id) => {
        setModalState({
            type: 'confirmDeleteWeight',
            data: { id: id, message: "Are you sure you want to delete this weight entry?" }
        });
    };

    const confirmDeleteWeight = (id) => {
            ...
            try {
                ...
                if (data.settings && data.pastWorkouts && data.templates && data.exercisePRs && data.bodyStats) {
                    setModalState({
                        ...
                        data: {
                            parsedData: data,
                            message: "Are you sure you want to import this data? This will overwrite all current data."
                        }
                    });
                } else {
                    setModalState({
                        type: 'alert',
                        data: { title: "Import Error", message: "Invalid backup file." }
                    });
                }
            } catch (error) {
                setModalState({
                    type: 'alert',
                    data: { title: "Import Error", message: "Could not parse the backup file. It may be corrupted." }
                });
            }
        };
        ...
        <div className="bg-gray-900 text-white min-h-screen font-sans">
            <div className="max-w-xl mx-auto bg-gray-900 shadow-lg h-full">
                ...
                {modalState.type === 'addExerciseToTemplate' && (
                    <Modal onClose={() => setModalState({ type: null })} title="Add Exercise to Template">
                        <form onSubmit={addExerciseToTemplateBuilder} className="space-y-4">
                            <input type="text" value={newExerciseName} onChange={(e) => setNewExerciseName(e.target.value)} placeholder="e.g., Squats" className="w-full bg-gray-700 text-white p-3 rounded-lg" autoFocus/>
                            <select value={newExerciseMuscleGroup} onChange={(e) => setNewExerciseMuscleGroup(e.target.value)} className="w-full bg-gray-700 text-white p-3 rounded-lg">
                                {MUSCLE_GROUPS.map(g => <option key={g} value={g}>{g}</option>)}
                            </select>
                            <button type="submit" className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 rounded-lg">Add</button>
                ...
                {modalState.type === 'confirmDeleteWeight' && (
                    <ConfirmPromptModal
                        title="Confirm Deletion"
                        message={modalState.data.message}
                        onConfirm={() => confirmDeleteWeight(modalState.data.id)}
                ...
                {modalState.type === 'alert' && (
                    <ConfirmPromptModal // Reusing for alert, just providing onConfirm (OK button)
